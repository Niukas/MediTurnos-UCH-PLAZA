            ...
            <form id="filter-form" method="GET" action="" class="m-0 flex flex-col gap-4">
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    <div class="relative w-full sm:flex-1">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate/50"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></div>
                        <input type="search" name="q" placeholder="Buscar por paciente, médico..." value="<?= h($_GET['q'] ?? '') ?>" class="w-full pl-10 pr-4 py-2.5 bg-[#F8FAFC] border border-gray-200/80 rounded-xl text-sm font-medium text-charcoal placeholder-gray-400 focus:outline-none focus:border-slate shadow-sm transition-colors">
                    </div>
                    <div class="relative w-full sm:w-64">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate/50"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div>
                        <input type="text" id="calendario" name="fecha" value="<?= h($_GET['fecha'] ?? '') ?>" placeholder="Seleccionar un rango..." class="w-full pl-10 pr-4 py-2.5 bg-[#F8FAFC] border border-gray-200/80 rounded-xl text-sm font-medium text-charcoal placeholder-gray-400 focus:outline-none focus:border-slate shadow-sm transition-colors cursor-pointer">
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row items-center gap-4">
                ...
                <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
                    ...
                    <?php if (!empty($_GET['q']) || !empty($_GET['especialidad']) || !empty($_GET['estado']) || !empty($_GET['fecha'])): ?>
                    <a href="dashboardAdminTurnos.php" class="w-full sm:w-auto text-center bg-white hover:bg-gray-100 text-slate font-bold py-2.5 px-6 rounded-xl text-sm transition-all duration-200 border border-gray-200 shadow-sm">Limpiar</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-50 bg-white">
                        <?php if (!isset($listadoTurnos) || empty($listadoTurnos)): ?>
                            <tr><td colspan="5" class="py-10 px-6 text-center text-sm text-slate italic font-medium">No hay turnos agendados con los criterios seleccionados.</td></tr>
                        <?php else: ?>
                            <?php foreach ($listadoTurnos as $t): ?>
                                <tr class="hover:bg-ghost/30 transition-colors group">
                                    <td class="py-4 px-6 align-middle">
                                        ...
                                        <div class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-ghost border border-gray-200 text-[0.7rem] font-mono text-slate mt-1"><svg class="w-3 h-3 text-slate" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><?= isset($t['hora_inicio']) ? substr($t['hora_inicio'], 0, 5) : '00:00' ?> hs</div>
                                    </td>
                                    <td class="py-4 px-6 align-middle">
                                        <div class="flex items-center gap-2">
                                            <div class="w-7 h-7 rounded-full bg-slate/10 text-slate flex items-center justify-center font-bold text-[0.65rem] uppercase"><?= substr(h($t['paciente_nombre'] ?? 'P'), 0, 1) . substr(h($t['paciente_apellido'] ?? 'A'), 0, 1) ?></div>
                                            <span class="text-sm font-bold text-charcoal"><?= h(($t['paciente_nombre'] ?? '') . ' ' . ($t['paciente_apellido'] ?? '')) ?></span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-6 text-sm text-slate font-medium align-middle">Dr/a. <?= h(($t['medico_nombre'] ?? '') . ' ' . ($t['medico_apellido'] ?? '')) ?></td>
                                    <td class="py-4 px-6 align-middle"><span class="bg-ghost border border-gray-200 text-charcoal text-[0.65rem] font-bold px-2 py-0.5 rounded uppercase tracking-wider"><?= h($t['especialidad'] ?? 'General') ?></span></td>
                                    <td class="py-4 px-6 align-middle text-right">
                                        <form method="POST" action="dashboardAdminTurnos.php" class="m-0 flex items-center justify-end gap-2">
                                            <input type="hidden" name="accion" value="cambiarEstado"><input type="hidden" name="id_turno" value="<?= $t['id_turno'] ?>">
                                            <?php
                                            ...
                                            <div class="relative">
                                                <select name="estado" class="pl-3 pr-8 py-1.5 rounded-lg text-[0.7rem] font-bold uppercase tracking-wider border <?= $selectColors ?> outline-none focus:ring-2 appearance-none cursor-pointer transition-all shadow-sm">
                                                    <?php foreach (['pendiente', 'confirmado', 'cancelado', 'realizado'] as $e): ?>
                                                        <option value="<?= $e ?>" <?= $estadoActual === $e ? 'selected' : '' ?> class="bg-white text-charcoal normal-case tracking-normal"><?= ucfirst($e) ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <div class="absolute inset-y-0 right-0 pr-2 flex items-center pointer-events-none opacity-50 text-slate"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div>
                                            </div>
                                            <button type="submit" class="bg-white border border-gray-200 text-slate hover:border-charcoal hover:text-charcoal hover:shadow-sm w-7 h-7 rounded-lg flex items-center justify-center transition-all" title="Guardar estado"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg></button>
                                            <button type="button" data-id-turno="<?= $t['id_turno'] ?>" class="historial-btn bg-white border border-gray-200 text-slate hover:border-charcoal hover:text-charcoal hover:shadow-sm w-7 h-7 rounded-lg flex items-center justify-center transition-all" title="Ver historial del turno"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg></button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div class="mt-8 flex justify-center">
                <nav class="inline-flex items-center gap-1.5 bg-white border border-gray-200/80 rounded-xl p-1.5 shadow-sm">
                    <?php if ($paginaActual > 1): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['pagina' => $paginaActual - 1])) ?>" class="flex items-center justify-center px-3 py-1.5 text-xs font-bold text-slate hover:bg-ghost rounded-lg transition-colors">Ant</a>
                    <?php endif; ?>
                    ...
                    if ($inicio > 1):
                    ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['pagina' => 1])) ?>" class="w-8 h-8 flex items-center justify-center text-xs font-bold text-slate hover:bg-ghost rounded-lg transition-colors">1</a>
                    ...
                    <?php if ($fin < $totalPaginas): ?>
                        ...
                        <a href="?<?= http_build_query(array_merge($_GET, ['pagina' => $totalPaginas])) ?>" class="w-8 h-8 flex items-center justify-center text-xs font-bold text-slate hover:bg-ghost rounded-lg transition-colors"><?= $totalPaginas ?></a>
                    <?php endif; ?>
                    
                    <?php if ($paginaActual < $totalPaginas): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['pagina' => $paginaActual + 1])) ?>" class="flex items-center justify-center px-3 py-1.5 text-xs font-bold text-slate hover:bg-ghost rounded-lg transition-colors">Sig</a>
                    <?php endif; ?>
                </nav>
            </div>
        <?php endif; ?>
    ...
    <div id="historial-modal" class="fixed inset-0 bg-black/30 backdrop-blur-sm z-[150] hidden items-center justify-center animate-fadeIn">
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-lg border border-gray-200/80 p-6 m-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <button id="close-modal-btn" class="text-slate hover:text-charcoal transition-colors"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
            </div>
